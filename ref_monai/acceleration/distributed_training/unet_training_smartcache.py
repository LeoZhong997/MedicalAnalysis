...
def train(args):
    # disable logging for process except 0 on every node
    ...
    # it can avoid duplicated caching content in each rank,
    # but will not do global shuffle before every epoch
    data_part = partition_dataset(
        data=train_files,
        num_partitions=dist.get_world_size(),  # 返回分布式环境下总共的进程数world_size
        shuffle=True,
        even_divisible=True,
    )[dist.get_rank()]  # 返回当前进程在分布式环境下的唯一标识符，通常被称为进程的rank。rank的范围是从0到 world_size-1
    ...
    # and use RandCropByPosNegLabeld to generate 2 x 4 images for network training
...
def main():
    ...
    parser.add_argument("-d", "--dir", default="./test_data", type=str, help="directory to create random data")
    # must parse the command-line argument: ``--local_rank=LOCAL_PROCESS_RANK``,
    # which will be provided by DDP
